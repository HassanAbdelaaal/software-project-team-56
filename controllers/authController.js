...
exports.requestPasswordResetOTP = async (req, res, next) => {
  try {
    ...
    console.log(`OTP generated for ${email}: ${otp} (showing in development only)`);

    try {
      // Send OTP via email with enhanced error handling
      console.log('Attempting to send OTP email...');
      
      const emailInfo = await emailService.sendOTP(user.email, otp);
      console.log('Email send result:', emailInfo);
      ...
      return res.status(500).json({
        ...
        errorDetail: process.env.NODE_ENV === 'development' ? emailError.message : undefined
      });
    }
  } catch (error) {
...
/**
 * @desc    Verify OTP
 * @route   POST /api/v1/forgetPassword/verify
 * @access  Public
 */
exports.verifyOTP = async (req, res, next) => {
  try {
    ...
    // OTP is valid, but don't reset it yet - user still needs to set new password
    res.status(200).json({
      success: true,
